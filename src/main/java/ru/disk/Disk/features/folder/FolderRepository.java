...
public interface FolderRepository extends JpaRepository<FolderEntity, Long> {
    ...
    @Query("SELECT u FROM folders u WHERE folder_id = ?1 AND user_id = ?2 AND in_basket = false")
    Page<FolderEntity> findByFolderId(Long folderId, Long userId, Pageable pageable);

    @Query("SELECT u FROM folders u WHERE folder_id = NULL AND user_id = ?1 AND in_basket = false")
